...
export function inferMarketImpacts(
  ...
  if (event.marketChannels?.includes("rates") || /fed|fomc|ecb|boj|bok|treasury|bond|국채|금리|연준|한국은행/.test(text)) {
  ...
  const energyDirection = detectDirection(
    text,
    /hormuz|supply cut|production cut|sanction|attack|war|shipping disruption|호르무즈|공급 감소|감산|제재|공격|전쟁|운송 차질/i,
    /ceasefire|supply increase|production increase|demand slowdown|휴전|증산|수요 둔화/i,
  );
  if (event.marketChannels?.includes("energy") || /oil|crude|brent|wti|opec|gas|energy|원유|유가|천연가스|에너지/.test(text)) {
  ...
  const equityDirection = detectDirection(
    ...
    /earnings miss|guidance cut|rate hike|sanction|war|tariff|recession|실적 하회|가이던스 하향|금리 인상|제재|전쟁|관세|침체/i,
  );
  if (event.marketChannels?.includes("equities") || /stocks?|equity|nasdaq|s&p|kospi|semiconductor|nvidia|주식|증시|반도체|엔비디아/.test(text)) {
  ...
  if (event.marketChannels?.includes("fx") || /dollar|usd|krw|yen|jpy|yuan|cny|환율|달러|원화|엔화|위안/.test(text)) {
  ...
  if (event.marketChannels?.includes("crypto") || /bitcoin|btc|crypto|ethereum|비트코인|가상자산/.test(text)) {
  ...
  if (/gdp|growth|inflation|cpi|pce|employment|payroll|unemployment|성장|물가|고용|실업/.test(text)) {
